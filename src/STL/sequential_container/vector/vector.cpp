...
int main()
{
    ...
    // 3) front()
    // cout << "empty v1.front()= " << v1.front() << endl; // err: Segmentation fault
    ...
    // 计算所有元素的和
    // accumulate 函数，前两个形参指定要累加元素的范围，第三个形参
    // 则是累加的初始值，其返回值是初始值的类型
    vector<int> nums = {1, 2, 4};
    cout << accumulate(nums.begin(), nums.end(), 0) << endl;  // 7
    cout << accumulate(nums.begin(), nums.end(), 10) << endl; // 17
    return 0;
}